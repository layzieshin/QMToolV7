#!/usr/bin/env python3
"""Fail-closed gate for signature PostgreSQL migrations (AP-029 PG01-A)."""
...
from modules.signature.postgres_schema import (  # noqa: E402
    PostgresSchemaError,
    discover_migrations,
)

PG_MIGRATIONS_REL = Path("modules/signature/postgres/migrations")
...
def evaluate_signature_postgres_migration_gate(
    *,
    base_ref: str,
    root: Path = ROOT,
) -> dict[str, object]:
    ...
    checks["provision_signature_schema_present"] = provision_path.is_file()
    ...
    checks["provision_reuses_shared_roles"] = (
        ...
        and "CREATE SCHEMA SIGNATURE AUTHORIZATION QMTOOL_MIGRATOR" in provision_upper
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Signature PostgreSQL migration gate")
    parser.add_argument(
        "--base-ref",
        required=True,
    ...
    result = evaluate_signature_postgres_migration_gate(base_ref=args.base_ref)
