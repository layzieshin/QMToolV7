#!/usr/bin/env python3
"""Fail-closed gate for registry PostgreSQL migrations (AP-029 PG01-A)."""
...
from modules.registry.postgres_schema import (  # noqa: E402
    PostgresSchemaError,
    discover_migrations,
)

PG_MIGRATIONS_REL = Path("modules/registry/postgres/migrations")
...
def evaluate_registry_postgres_migration_gate(
    *,
    base_ref: str,
    root: Path = ROOT,
) -> dict[str, object]:
    ...
    checks["provision_registry_schema_present"] = provision_path.is_file()
    ...
    checks["provision_reuses_shared_roles"] = (
        ...
        and "CREATE SCHEMA REGISTRY AUTHORIZATION QMTOOL_MIGRATOR" in provision_upper
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Registry PostgreSQL migration gate")
    parser.add_argument(
        "--base-ref",
        required=True,
    ...
    result = evaluate_registry_postgres_migration_gate(base_ref=args.base_ref)
