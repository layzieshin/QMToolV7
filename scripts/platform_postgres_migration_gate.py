#!/usr/bin/env python3
"""Fail-closed gate for platform PostgreSQL migrations (AP-029 PG00-A)."""
...
from qm_platform.persistence.postgres_schema import (  # noqa: E402
    PostgresSchemaError,
    discover_migrations,
)

PG_MIGRATIONS_REL = Path("qm_platform/persistence/postgres/migrations")
PROVISION_REL = Path("qm_platform/persistence/postgres/provision_platform_schema.sql")
...
def evaluate_platform_postgres_migration_gate(
    *,
    base_ref: str,
    root: Path = ROOT,
) -> dict[str, object]:
    ...
    checks["provision_platform_schema_present"] = provision_path.is_file()
    ...
    checks["provision_reuses_shared_roles"] = (
        ...
        and "CREATE SCHEMA PLATFORM AUTHORIZATION QMTOOL_MIGRATOR" in provision_upper
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Platform PostgreSQL migration gate"
    )
    parser.add_argument(
        "--base-ref",
        required=True,
    ...
    result = evaluate_platform_postgres_migration_gate(base_ref=args.base_ref)
