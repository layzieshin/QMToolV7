...
def test_draft_and_artifact_access_is_parent_authorized(tmp_path: Path) -> None:
    ...
    imported = client.post(
        "/api/v1/documents/versions/DOC-AUTH-DRAFT/1/import-pdf",
    ...
    artifact_id = client.get(
        "/api/v1/documents/versions/DOC-AUTH-DRAFT/1/artifacts", headers=_auth(qmb)
    ).json()[0]["artifact_id"]

    assert client.get("/api/v1/documents/versions/DOC-AUTH-DRAFT/1", headers=_auth(observer)).status_code == 404
    assert client.get(
        f"/api/v1/documents/versions/DOC-AUTH-DRAFT/1/artifacts", headers=_auth(observer)
    ).status_code == 404
    assert client.get(f"/api/v1/documents/artifacts/{artifact_id}/content", headers=_auth(observer)).status_code == 404
...
def test_approved_document_and_current_pdf_are_readable_by_observer(tmp_path: Path) -> None:
    ...
    edited = client.post(
        "/api/v1/documents/versions/DOC-AUTH-APPROVED/1/workflow/editing-complete",
        headers=_mutation_headers(tokens["editor"], tokens["state_response"]),
    )
    reviewed = client.post(
        "/api/v1/documents/versions/DOC-AUTH-APPROVED/1/workflow/review/accept",
        headers=_mutation_headers(tokens["reviewer"], edited),
    )
    approved = client.post(
        "/api/v1/documents/versions/DOC-AUTH-APPROVED/1/workflow/approval/accept",
        headers=_mutation_headers(tokens["approver"], reviewed),
    )
    assert approved.status_code == 200, approved.text
    observer = _login(client, "observer", "observerpass01")
    visible = client.get("/api/v1/documents/versions/DOC-AUTH-APPROVED/1", headers=_auth(observer))
    assert visible.status_code == 200, visible.text
    artifacts = client.get(
        "/api/v1/documents/versions/DOC-AUTH-APPROVED/1/artifacts", headers=_auth(observer)
    )
...
def test_non_reviewer_cannot_accept_review(tmp_path: Path) -> None:
    ...
    edited = client.post(
        "/api/v1/documents/versions/DOC-AUTH-REV/1/workflow/editing-complete",
        headers=_mutation_headers(tokens["editor"], tokens["state_response"]),
    )
    assert edited.status_code == 200, edited.text
    denied = client.post(
        "/api/v1/documents/versions/DOC-AUTH-REV/1/workflow/review/accept",
...
def test_non_qmb_cannot_archive_or_new_version(tmp_path: Path) -> None:
    ...
    edited = client.post(
        "/api/v1/documents/versions/DOC-AUTH-LIFE/1/workflow/editing-complete",
        headers=_mutation_headers(tokens["editor"], tokens["state_response"]),
    )
    reviewed = client.post(
        "/api/v1/documents/versions/DOC-AUTH-LIFE/1/workflow/review/accept",
        headers=_mutation_headers(tokens["reviewer"], edited),
    )
    approved = client.post(
        "/api/v1/documents/versions/DOC-AUTH-LIFE/1/workflow/approval/accept",
        headers=_mutation_headers(tokens["approver"], reviewed),
    )
    assert approved.status_code == 200, approved.text
    denied_archive = client.post(
        "/api/v1/documents/versions/DOC-AUTH-LIFE/1/lifecycle/archive",
    ...
    denied_new = client.post(
        "/api/v1/documents/versions/DOC-AUTH-LIFE/1/lifecycle/new-version-after-archive",
...
def test_stale_if_match_on_new_version_returns_409_without_mutation(tmp_path: Path) -> None:
    ...
    edited = client.post(
        "/api/v1/documents/versions/DOC-AUTH-STALE-NV/1/workflow/editing-complete",
        headers=_mutation_headers(tokens["editor"], tokens["state_response"]),
    )
    reviewed = client.post(
        "/api/v1/documents/versions/DOC-AUTH-STALE-NV/1/workflow/review/accept",
        headers=_mutation_headers(tokens["reviewer"], edited),
    )
    approved = client.post(
        "/api/v1/documents/versions/DOC-AUTH-STALE-NV/1/workflow/approval/accept",
        headers=_mutation_headers(tokens["approver"], reviewed),
    )
    qmb = _login(client, "qmb", "qmbpass001")
    archived = client.post(
        "/api/v1/documents/versions/DOC-AUTH-STALE-NV/1/lifecycle/archive",
        headers=_mutation_headers(qmb, approved),
    )
    assert archived.status_code == 200, archived.text
    stale_headers = _mutation_headers(qmb, archived)
    bumped = client.patch(
        "/api/v1/documents/versions/DOC-AUTH-STALE-NV/1/metadata",
        headers=_mutation_headers(qmb, archived),
    ...
    denied = client.post(
        "/api/v1/documents/versions/DOC-AUTH-STALE-NV/1/lifecycle/new-version-after-archive",
        headers=stale_headers,
        json={"next_version": 2},
    )
    assert denied.status_code == 409, denied.text
    assert denied.json()["detail"]["error"] == "document_conflict"
    missing = client.get("/api/v1/documents/versions/DOC-AUTH-STALE-NV/2", headers=_auth(qmb))
...
def test_new_version_consumes_source_etag_and_links_successor(tmp_path: Path) -> None:
    ...
    edited = client.post(
        "/api/v1/documents/versions/DOC-AUTH-NV-CAS/1/workflow/editing-complete",
        headers=_mutation_headers(tokens["editor"], tokens["state_response"]),
    )
    reviewed = client.post(
        "/api/v1/documents/versions/DOC-AUTH-NV-CAS/1/workflow/review/accept",
        headers=_mutation_headers(tokens["reviewer"], edited),
    )
    approved = client.post(
        "/api/v1/documents/versions/DOC-AUTH-NV-CAS/1/workflow/approval/accept",
        headers=_mutation_headers(tokens["approver"], reviewed),
    )
    qmb = _login(client, "qmb", "qmbpass001")
    archived = client.post(
        "/api/v1/documents/versions/DOC-AUTH-NV-CAS/1/lifecycle/archive",
        headers=_mutation_headers(qmb, approved),
    )
    assert archived.status_code == 200, archived.text
    source_etag = archived.json()["etag"]
    created = client.post(
        "/api/v1/documents/versions/DOC-AUTH-NV-CAS/1/lifecycle/new-version-after-archive",
        headers=_mutation_headers(qmb, archived),
        json={"next_version": 2},
    )
    assert created.status_code == 200, created.text
    ...
    source = client.get("/api/v1/documents/versions/DOC-AUTH-NV-CAS/1", headers=_auth(qmb))
    ...
    missing = client.get("/api/v1/documents/versions/DOC-AUTH-NV-CAS/3", headers=_auth(qmb))
    assert missing.status_code == 404
...
def test_non_qmb_cannot_update_header(tmp_path: Path) -> None:
    ...
    assert created.status_code == 200, created.text
    header_before = client.get("/api/v1/documents/headers/DOC-AUTH-HDR", headers=_auth(qmb))
    assert header_before.status_code == 200
    editor = _login(client, "editor", "editorpass01")
    denied = client.put(
        "/api/v1/documents/headers/DOC-AUTH-HDR",
...
def test_open_source_denied_for_assigned_non_editor(tmp_path: Path) -> None:
    ...
    listed = client.get(
        "/api/v1/documents/versions/DOC-AUTH-OPEN/1/artifacts",
        headers=_auth(tokens["editor"]),
    )
    assert listed.status_code == 200, listed.text
    artifact_id = listed.json()[0]["artifact_id"]
    allowed = client.get(f"/api/v1/documents/artifacts/{artifact_id}/content", headers=_auth(tokens["editor"]))
    assert allowed.status_code == 200, allowed.text
    denied = client.get(f"/api/v1/documents/artifacts/{artifact_id}/content", headers=_auth(tokens["reviewer"]))
...
def test_assign_start_abort_http_public_boundaries(tmp_path: Path) -> None:
    ...
    assert created.status_code == 200, created.text
    imported = client.post(
        "/api/v1/documents/versions/DOC-AUTH-ASA/1/import-pdf",
    ...
    denied_assign = client.post(
        "/api/v1/documents/versions/DOC-AUTH-ASA/1/workflow/assign-roles",
    ...
    denied_start = client.post(
        "/api/v1/documents/versions/DOC-AUTH-ASA/1/workflow/start",
    ...
    denied_abort = client.post(
        "/api/v1/documents/versions/DOC-AUTH-ASA/1/workflow/abort",
...
def test_review_and_approval_reject_http(tmp_path: Path) -> None:
    ...
    edited = client.post(
        "/api/v1/documents/versions/DOC-AUTH-REJ/1/workflow/editing-complete",
        headers=_mutation_headers(tokens["editor"], tokens["state_response"]),
    )
    assert edited.status_code == 200, edited.text
    denied_reject = client.post(
        "/api/v1/documents/versions/DOC-AUTH-REJ/1/workflow/review/reject",
        headers=_mutation_headers(tokens["editor"], edited),
    ...
    reviewed = client.post(
        "/api/v1/documents/versions/DOC-AUTH-REJ/1/workflow/review/accept",
        headers=_mutation_headers(tokens["reviewer"], edited2),
    )
    assert reviewed.status_code == 200, reviewed.text
    denied_approval_reject = client.post(
        "/api/v1/documents/versions/DOC-AUTH-REJ/1/workflow/approval/reject",
...
def test_hidden_header_is_404_for_observer(tmp_path: Path) -> None:
    ...
    assert created.status_code == 200, created.text
    assert client.get("/api/v1/documents/headers/DOC-AUTH-HIDDEN-HDR", headers=_auth(qmb)).status_code == 200
    hidden = client.get("/api/v1/documents/headers/DOC-AUTH-HIDDEN-HDR", headers=_auth(observer))
...
def test_unauthorized_stale_mutations_are_404_without_current_state(tmp_path: Path) -> None:
    ...
    assign = client.post(
        "/api/v1/documents/versions/DOC-AUTH-STALE-HIDE/1/workflow/assign-roles",
        headers=headers,
        json={"editors": ["editor"], "reviewers": ["reviewer"], "approvers": ["approver"]},
    )
    imported = client.post(
        "/api/v1/documents/versions/DOC-AUTH-STALE-HIDE/1/import-pdf",
        headers={**headers, "Content-Type": "application/pdf"},
        content=_minimal_pdf_bytes(),
    )
    commented = client.post(
        "/api/v1/documents/versions/DOC-AUTH-STALE-HIDE/1/comments",
        headers=headers,
        json={"context": "PDF", "page_number": 1, "comment_text": "secret"},
    )
    started = client.post(
        "/api/v1/documents/versions/DOC-AUTH-STALE-HIDE/1/workflow/start",
        headers=headers,
        json={"profile_id": "http_flow_profile"},
    )
...
def test_authorized_stale_mutation_still_returns_409(tmp_path: Path) -> None:
    ...
    first = client.post(
        "/api/v1/documents/versions/DOC-AUTH-STALE-OK/1/workflow/assign-roles",
...
def test_observer_cannot_create_from_template_when_target_missing(
    ...
    denied = client.post(
        "/api/v1/documents/versions/DOC-TPL-MISS/1/create-from-template",
        headers={**_auth(observer), "Content-Type": _TEMPLATE_CT},
        content=_MINIMAL_DOTX,
    )
    assert denied.status_code == 403, denied.text
    assert client.get("/api/v1/documents/versions/DOC-TPL-MISS/1", headers=_auth(observer)).status_code == 404
    assert client.get("/api/v1/documents/versions/DOC-TPL-MISS/1", headers=_auth(qmb)).status_code == 404
...
def test_visible_non_owner_create_from_template_hides_stale_conflict(
    ...
    assert created.status_code == 200, created.text
    assigned = client.post(
        "/api/v1/documents/versions/DOC-TPL-VIS/1/workflow/assign-roles",
        headers=_mutation_headers(qmb, created),
        json={"editors": ["editor"], "reviewers": ["reviewer"], "approvers": ["approver"]},
    )
    assert assigned.status_code == 200, assigned.text
    imported = client.post(
        "/api/v1/documents/versions/DOC-TPL-VIS/1/import-pdf",
    ...
    stale = client.post(
        "/api/v1/documents/versions/DOC-TPL-VIS/1/create-from-template",
...
def test_authorized_create_from_template_stale_still_returns_409(
    ...
    assert created.status_code == 200, created.text
    assigned = client.post(
        "/api/v1/documents/versions/DOC-TPL-OWN/1/workflow/assign-roles",
        headers=_mutation_headers(qmb, created),
        json={"editors": ["editor"], "reviewers": ["reviewer"], "approvers": ["approver"]},
    )
    assert assigned.status_code == 200, assigned.text
    ...
    stale = client.post(
        "/api/v1/documents/versions/DOC-TPL-OWN/1/create-from-template",
