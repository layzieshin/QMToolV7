"""Git-backed tests for the documents PostgreSQL migration gate."""
...
from scripts import documents_postgres_migration_gate as gate


MIGRATIONS_REL = Path("modules/documents/postgres/migrations")
...
def scratch_repo(tmp_path: Path) -> Path:
    ...
    _write(
        root / PROVISION_REL,
        "CREATE SCHEMA documents AUTHORIZATION qmtool_migrator;\n"
...
def test_non_append_version_is_rejected(scratch_repo: Path) -> None:
    _write(scratch_repo / MIGRATIONS_REL / "0000_prepended.sql", "SELECT 0;\n")

    result = gate.evaluate_documents_postgres_migration_gate(base_ref="HEAD", root=scratch_repo)
...
def test_correctly_appended_migration_is_accepted(scratch_repo: Path) -> None:
    _write(scratch_repo / MIGRATIONS_REL / "0002_workflow_profiles.sql", "SELECT 2;\n")
    _write(scratch_repo / MIGRATIONS_REL / "0003_grant_history_select.sql", "SELECT 3;\n")
    _write_packaging(
        scratch_repo,
        ("0001_initial.sql", "0002_workflow_profiles.sql", "0003_grant_history_select.sql"),
    )

    result = gate.evaluate_documents_postgres_migration_gate(base_ref="HEAD", root=scratch_repo)

    assert result["ok"] is True
    assert result["checks"]["migration_chain_valid"] is True
    assert result["checks"]["pg_artifacts_are_bundled"] is True
