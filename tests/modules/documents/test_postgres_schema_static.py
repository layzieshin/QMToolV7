...
def test_public_provision_postgres_schema_delegates_once_with_admin_dsn(monkeypatch) -> None:
    ...
    from modules.documents.api import provision_postgres_schema
    ...
    monkeypatch.setattr(
        "modules.documents.postgres_schema.provision_documents_schema",
        fake_provision,
    )
    assert list(inspect.signature(provision_postgres_schema).parameters) == ["admin_dsn"]
    assert "provision_postgres_schema" in documents_api.__all__
    assert "migrate_postgres_schema" in documents_api.__all__
    assert provision_postgres_schema("admin-dsn") is None
    assert calls == [("admin-dsn", {})]

    owner_exc = RuntimeError("documents provision failed")
...
def test_public_migrate_postgres_schema_returns_owner_result_and_propagates(monkeypatch) -> None:
    ...
    from modules.documents import api as documents_api
    from modules.documents.api import migrate_postgres_schema
    ...
    monkeypatch.setattr(
        "modules.documents.postgres_schema.migrate_documents_schema",
        fake_migrate,
    )
    ...
    assert "provision_postgres_schema" in documents_api.__all__
    assert "migrate_postgres_schema" in documents_api.__all__
    assert migrate_postgres_schema("migrator-dsn") == 4
    assert calls == [("migrator-dsn", {})]

    owner_exc = RuntimeError("documents migrate failed")
