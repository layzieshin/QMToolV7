"""PostgreSQL schema applicator for Signature only (AP-029 PG01-A).
...
2. Administrative ``provision_signature_schema.sql`` (empty signature schema)
3. ``migrate_signature_schema`` as a LOGIN member of ``qmtool_migrator``
"""
...
SCHEMA_NAME = "signature"
MIGRATIONS_TABLE = "_qm_schema_migrations"
MIGRATOR_ROLE = "qmtool_migrator"
RUNTIME_ROLE = "qmtool_runtime"
ADVISORY_LOCK_KEY = 0x5154_4D5F_5349_474E  # "QTM_SIGN"
POSTGRES_ROOT = Path(__file__).resolve().parent / "postgres"
MIGRATIONS_DIR = POSTGRES_ROOT / "migrations"
PROVISION_SIGNATURE_SCHEMA_PATH = POSTGRES_ROOT / "provision_signature_schema.sql"

_MIGRATION_NAME_RE = re.compile(r"^(\d{4})_([a-z0-9_]+)\.sql$")

EXPECTED_TABLES = frozenset({'_qm_schema_migrations', 'signature_assets', 'user_active_signatures', 'user_signature_templates'})
...
EXPECTED_SIGNATURE_ASSETS_COLUMNS = frozenset(
    {
...
EXPECTED_USER_ACTIVE_SIGNATURES_COLUMNS = frozenset(
    {
'asset_id', 'owner_user_id', 'updated_at'
    })
EXPECTED_CHECK_CONSTRAINTS = frozenset({})


class PostgresSchemaError(RuntimeError):
    """Raised when Signature PostgreSQL schema migration cannot proceed safely."""
...
def provision_signature_schema(admin_dsn: str) -> None:
    script = PROVISION_SIGNATURE_SCHEMA_PATH.read_text(encoding="utf-8")
...
def _validate_runtime_identity(conn: psycopg.Connection) -> None:
    ...
    if (
        ...
        raise PostgresSchemaError(
            "signature readiness requires a LOGIN member of qmtool_runtime "
...
def _require_bootstrap_or_history(conn: psycopg.Connection, applied: tuple[AppliedMigration, ...]) -> None:
    ...
    if not _schema_exists(conn):
        raise PostgresSchemaError("schema signature is missing; run provision_signature_schema.sql before migrate")
    owner = conn.execute(
        """
        SELECT r.rolname
        FROM pg_namespace n
    ...
    if owner is None or owner[0] != MIGRATOR_ROLE:
        raise PostgresSchemaError("bootstrap schema signature must be owned by qmtool_migrator")
    ...
    if relations or functions or types:
        raise PostgresSchemaError(
            "populated unversioned schema signature refused; "
...
def _validate_role_contract(
    ...
    *,
    ...
    if public_schema_access:
        raise PostgresSchemaError("PUBLIC must not have USAGE or CREATE on schema signature")
...
def _validate_schema_contracts(
    ...
    *,
    ...
    missing_signature_assets = EXPECTED_SIGNATURE_ASSETS_COLUMNS - columns_for("signature_assets")
    ...
    missing_user_active_signatures = EXPECTED_USER_ACTIVE_SIGNATURES_COLUMNS - columns_for("user_active_signatures")
    if missing_user_active_signatures:
        raise PostgresSchemaError(f"user_active_signatures missing columns: {sorted(missing_user_active_signatures)}")
    ...
    runtime_tables = ('signature_assets', 'user_signature_templates', 'user_active_signatures') if require_full else ('signature_assets', 'user_signature_templates', 'user_active_signatures')
    ...
    *,
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        if locked is None or not locked[0]:
            raise PostgresSchemaError("signature schema migration already in progress (advisory lock held)")
        try:
...
def assert_runtime_schema_ready(dsn: str, *, migrations_dir: Path | None = None) -> int:
    ...
    with _runtime_connection_for_schema(dsn) as conn:
        if not _history_table_exists(conn):
            raise PostgresSchemaError("signature schema history is missing")
        applied = _fetch_applied(conn)
        if not applied:
            raise PostgresSchemaError("signature schema history is empty")
        _validate_history_prefix(applied, steps)
        if applied[-1].version != target:
            raise PostgresSchemaError(
                f"signature schema not ready: applied version "
