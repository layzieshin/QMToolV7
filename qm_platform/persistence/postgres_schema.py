"""PostgreSQL schema applicator for platform persistence only (AP-029 PG00-A).

Parallel to AP-028 Usermanagement PostgreSQL — deliberately separate.
Not part of a public platform API surface.

Deployment order:
1. Administrative ``modules/usermanagement/postgres/provision_roles.sql`` (shared roles)
2. Administrative ``provision_platform_schema.sql`` (empty platform schema)
3. ``migrate_platform_schema`` as a LOGIN member of ``qmtool_migrator``
"""
...
# Stable advisory-lock key dedicated to platform schema migration (distinct from UM).
ADVISORY_LOCK_KEY = 0x5154_4D5F_504C_4154  # "QTM_PLAT"
POSTGRES_ROOT = Path(__file__).resolve().parent / "postgres"
MIGRATIONS_DIR = POSTGRES_ROOT / "migrations"
PROVISION_PLATFORM_SCHEMA_PATH = POSTGRES_ROOT / "provision_platform_schema.sql"
...
EXPECTED_PLATFORM_SETTING_REVISIONS_COLUMNS = frozenset(
    {
        "revision_id",
        "scope_kind",
        "scope_id",
...
EXPECTED_PLATFORM_SETTINGS_INTEGRITY_COLUMNS = frozenset(
...
EXPECTED_CHECK_CONSTRAINTS = frozenset(
    {
        "platform_settings_scope_kind_module",
        "platform_settings_scope_id_matches_module",
        "platform_settings_revision_positive",
        "platform_setting_revisions_scope_kind_module",
        "platform_setting_revisions_scope_id_matches_module",
        "platform_setting_revisions_revision_positive",
    }
)
...
class PostgresSchemaError(RuntimeError):
    """Raised when platform PostgreSQL schema migration cannot proceed safely."""
...
def _validate_runtime_identity(conn: psycopg.Connection) -> None:
    ...
    if (
        ...
        raise PostgresSchemaError(
            "platform readiness requires a LOGIN member of qmtool_runtime "
...
def provision_platform_schema(admin_dsn: str) -> None:
    """Apply ``provision_platform_schema.sql`` as an administrative connection."""
    script = PROVISION_PLATFORM_SCHEMA_PATH.read_text(encoding="utf-8")
...
def _require_bootstrap_or_history(conn: psycopg.Connection, applied: tuple[AppliedMigration, ...]) -> None:
    ...
    if not _schema_exists(conn):
        raise PostgresSchemaError(
            "schema platform is missing; run provision_platform_schema.sql before migrate"
        )
    owner = conn.execute(
        """
        SELECT r.rolname
    ...
    if owner is None or owner[0] != MIGRATOR_ROLE:
        raise PostgresSchemaError(
            "bootstrap schema platform must be owned by qmtool_migrator"
        )
    ...
    if relations or functions or types:
        raise PostgresSchemaError(
            "populated unversioned schema platform refused; "
...
def schema_fingerprint(conn: psycopg.Connection) -> str:
    """Public wrapper for deterministic schema fingerprinting."""
...
def _validate_role_contract(
    ...
    *,
    ...
    if public_schema_access:
        raise PostgresSchemaError(
            "PUBLIC must not have USAGE or CREATE on schema platform"
        )

    required_dml = ("SELECT", "INSERT", "UPDATE", "DELETE")
    forbidden_table_privileges = ("TRUNCATE", "REFERENCES", "TRIGGER")
    runtime_tables = ("platform_settings", "platform_setting_revisions")
    if require_integrity:
        runtime_tables = (*runtime_tables, "platform_settings_integrity")
...
def _validate_schema_contracts(
    ...
    *,
    ...
    missing_settings = EXPECTED_PLATFORM_SETTINGS_COLUMNS - columns_for("platform_settings")
    if missing_settings:
        raise PostgresSchemaError(
            f"platform_settings missing columns: {sorted(missing_settings)}"
        )
    missing_revisions = (
        EXPECTED_PLATFORM_SETTING_REVISIONS_COLUMNS
        - columns_for("platform_setting_revisions")
    )
    if missing_revisions:
        raise PostgresSchemaError(
            f"platform_setting_revisions missing columns: {sorted(missing_revisions)}"
        )
    if require_integrity:
        missing_integrity = (
            EXPECTED_PLATFORM_SETTINGS_INTEGRITY_COLUMNS
            - columns_for("platform_settings_integrity")
        )
...
def migrate_platform_schema(
    dsn: str,
    *,
    migrations_dir: Path | None = None,
) -> int:
    """Apply pending platform PostgreSQL migrations. Returns resulting schema version."""
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        if locked is None or not locked[0]:
            raise PostgresSchemaError(
                "platform schema migration already in progress (advisory lock held)"
            )
        try:
...
def assert_runtime_schema_ready(dsn: str, *, migrations_dir: Path | None = None) -> int:
    ...
    with _runtime_connection_for_schema(dsn) as conn:
        if not _history_table_exists(conn):
            raise PostgresSchemaError("platform schema history is missing")
        applied = _fetch_applied(conn)
        if not applied:
            raise PostgresSchemaError("platform schema history is empty")
        _validate_history_prefix(applied, steps)
        if applied[-1].version != target:
            raise PostgresSchemaError(
                f"platform schema not ready: applied version "
