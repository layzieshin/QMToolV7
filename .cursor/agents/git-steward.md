...
gates, review verdict, verification evidence, and intended commit/PR metadata.

## Output contract

Return status/diff checks, exact staged/committed paths, commit SHA/message, push target, PR URL and
checks when applicable, merge result, and any conflict/CI blocker routed back to the coordinator.
